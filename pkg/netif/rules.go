...
type Rules struct {
	...
	dnsPort   string // DNS listen port, e.g. "53"
	setup     bool
}

// New creates a Rules manager.
//   lanSubnet: LAN subnet for SNAT MASQUERADE (e.g. "192.168.100.0/24")
//   dnsPort:   DNS listener port for iptables REDIRECT (e.g. "53")
func New(lanSubnet, dnsPort string) *Rules {
	if lanSubnet == "" {
	...
	return &Rules{lanSubnet: lanSubnet, dnsPort: dnsPort}
}
...
func (r *Rules) Setup() error {
	...
	_ = run("iptables", "-t", "nat", "-D", "PREROUTING", "-p", "udp", "--dport", "53", "-j", "REDIRECT", "--to-port", r.dnsPort)
	...
	cmds := [][]string{
		...
		// DNS transparent hijack: redirect UDP/53 traffic to Marmot's DNS server
		{"iptables", "-t", "nat", "-A", "PREROUTING", "-p", "udp", "--dport", "53", "-j", "REDIRECT", "--to-port", r.dnsPort},
		// DNS reply exception: avoid MASQUERADE rewriting src of local DNS responses
		{"iptables", "-t", "nat", "-A", "POSTROUTING", "-s", r.lanSubnet, "-p", "udp", "--sport", r.dnsPort, "-j", "ACCEPT"},
	...
	r.setup = true
	log.Info("network rules configured", map[string]interface{}{"lan_subnet": r.lanSubnet, "dns_port": r.dnsPort})
	return nil
}
...
func (r *Rules) Cleanup() error {
	...
	for i := 0; i < 10; i++ {
		if err := run("iptables", "-t", "nat", "-D", "PREROUTING", "-p", "udp", "--dport", "53", "-j", "REDIRECT", "--to-port", r.dnsPort); err != nil {
			break
		}
	}
	...
	for _, subnet := range []string{r.lanSubnet} {
		for i := 0; i < 10; i++ {
			if err := run("iptables", "-t", "nat", "-D", "POSTROUTING", "-s", subnet, "-p", "udp", "--sport", r.dnsPort, "-j", "ACCEPT"); err != nil {
				break
			}
		}
		for i := 0; i < 10; i++ {
