...
| 变更 | v0.2 | v0.3 |
|------|------|------|
| Conntrack Fast Cache | ❌ 无 | ✅ 新增（用户态↔BPF Map）|
...
| 流量路径 | 统一走 TProxy | **Fast Path / Slow Path 分流** |
...
### 2.2.2 Data Plane（数据面）— ⭐ Conntrack Fast Cache 新增

**职责**：流量捕获 → Flow Cache 查找（Fast Path）→ Conntrack Cache 同步 → TProxy (Slow Path)

#### 两阶段分类 + Conntrack Fast Cache
...
## 2.3 Conntrack Fast Cache 设计 ⭐ 新增
...
└────────────┘                  │ Match    │
                                └────┬─────┘
                                     │
                            ┌────────▼────────┐
                            │ Conntrack Cache │ ← 用户态写入
                            │ (Go map)        │
                            └────────┬────────┘
                                     │
                            ┌────────▼────────┐
                            │ BPF Flow Map    │ ← bpf_map_update_elem
                            │ (内核态)        │
                            └────────┬────────┘
                                     │
                            ┌────────▼────────┐
...
| 策略 | 说明 | 参数 |
|------|------|------|
| TTL 基淘汰 | 每个 flow 关联 TTL，过期即删 | TCP: 3600s, UDP: 120s |
| LRU 淘汰 | BPF Map 满时淘汰最久未命中项 | Map 上限: 65536 entries |
| 连接结束淘汰 | TCP FIN/RST 触发即时删除 | 内核态监测 |
| 规则变更刷新 | 用户态规则更新时清空 BPF Map | 优雅清空（inflight 连接保持）|

### 2.3.4 Flow BPF Map Schema
...
struct flow_value {
    ...
    __u8  outbound_tag[16]; // proxy outbound tag (proxy-jp / proxy-us / ...)
    __u32 expire_at;  // 过期时间戳 (jiffies / bpf_ktime)
    __u8  reserved[7];
};
...
### 2.3.5 Conntrack Cache Schema（用户态）

```go
// 用户态 Conntrack Cache（Go）
type FlowEntry struct {
    Key        FlowKey
    Action     Action            // Rule Engine 决策结果
    RuleID     string            // 命中的规则 ID
    CreatedAt  time.Time         // 创建时间
    LastHitAt  time.Time         // 最后命中时间（用于 LRU）
    ExpireAt   time.Time         // 过期时间
    HitCount   uint64            // 命中次数（统计用）
}
...
type ConntrackCache struct {
    entries map[FlowKey]*FlowEntry  // 主存储
    mu      sync.RWMutex
    stats   CacheStats
}
